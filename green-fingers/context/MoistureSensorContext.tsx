...
interface MoistureSensorContextProps {
  ...
  handleMoistureUpdate: (data: MoistureUpdateMessage) => void;
  handleNewSensor: (data: MoistureUpdateMessage & { plant_nickname?: string; user_plant_id?: number }) => void;
}
...
export const MoistureSensorProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchSensorWithHistory = async (sensorId: string): Promise<SensorWithHistory | undefined> => {
    ...
    try {
      if (sensorHistories[id]) {
        ...
        return { sensor, history: sensorHistories[id] };
      }
      const data = await apiGetSensorHistoryData(id);
      if (data?.history) {
        setSensorHistories(prev => ({ ...prev, [id]: data.history }));
      }
      return data;
    } catch (error) {
  ...
  const updateMoistureSensor = async (sensorId: number, sensorData: Partial<AddMoistureSensor>) => {
    try {
      await apiUpdateSensor(sensorId, sensorData);
      setSensors(prev =>
        prev.map(sensor => (sensor.id === sensorId ? { ...sensor, ...sensorData } : sensor))
      );
  ...
  const evaluateMoistureLevel = (expected: SoilMoisture, actual: SoilMoisture): Level => {
  ...
  const getMoistureLevel = (plantId: number, actualLevel: SoilMoisture): Level => {
    const plant = plants.find(p => p.id === plantId);
    if (!plant || !plant.neededMoisture) return "No Value";
    return evaluateMoistureLevel(plant.neededMoisture as SoilMoisture, actualLevel);
  };

  const handleMoistureUpdate = async (data: MoistureUpdateMessage) => {
    setSensors(prev =>
      prev.map(sensor =>
    ...
    if (updated?.history) {
      setSensorHistories(prev => ({ ...prev, [data.sensorId]: updated.history }));
    }
  };

  const handleNewSensor = async (data: MoistureUpdateMessage & {
  ...
  return (
    <MoistureSensorContext.Provider
      value={{
        ...
        fetchAllSensors,
        fetchSensor,
        fetchSensorWithHistory,
        getMoistureLevel,
        updateMoistureSensor,
        deleteMoistureSensor,
        getSensorWithHistory,
...
export const useMoistureSensors = () => {
  ...
  if (!context) throw new Error("useMoistureSensors must be used within MoistureSensorProvider");
  return context;
};
